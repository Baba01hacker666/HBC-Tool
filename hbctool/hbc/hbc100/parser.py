...
def parse(f):
    ...
    literalValueBuffer = f.read(literalValueBufferS[2])
    objKeyBuffer = f.read(objKeyBufferS[2])
    objShapeTableS = structure["objShapeTable"]
    objShapeTableS[2] = header["objShapeTableCount"]
    objShapeTable = f.read(objShapeTableS[1] // 8 * objShapeTableS[2])
